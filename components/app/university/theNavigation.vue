<template>
  <nav class="bg-white shadow-sm">
    <div class="max-w-7xl mx-auto">
      <div class="flex justify-between h-20">
        ...
        <div class="hidden sm:ml-6 sm:flex">
          <nuxt-link :to=" { name : 'app-university-dashboard' } " class="inline-flex items-center border-b-2 border-transparent text-sm font-medium leading-5 tracking-wide text-gray-500 hover:border-cyan-700 focus:outline-none transition duration-150 ease-in-out px-1 pt-1" :class="{ 'border-cyan-700 text-cyan-500' : this.$route.name === 'app-university-dashboard' }">Dashboard</nuxt-link>
          <nuxt-link :to=" { name : 'app-university-dashboard-students' } " class="inline-flex items-center border-b-2 border-transparent text-sm font-medium leading-5 tracking-wide text-gray-500 hover:border-cyan-700 focus:outline-none transition duration-150 ease-in-out px-1 pt-1 ml-12" :class="{ 'border-cyan-700 text-cyan-500' : this.$route.name === 'app-university-dashboard-students' }">Nos étudiants</nuxt-link>
          ...
          <div class="ml-3 relative">
            ...
            <div class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg" v-show="dropdownOpen">
              <div class="py-1 rounded-md bg-white shadow-xs" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
                <nuxt-link :to=" { name : '', params : { slug : '' } } " class="block text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out px-4 py-2">Mon profil</nuxt-link>
                <nuxt-link :to=" { name : '' } " class="block text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out px-4 py-2">Paramètres</nuxt-link>
    ...
    <div :class=" menuOpen ? 'block' : 'hidden' ">
      <div class="pt-2 pb-3">
        <nuxt-link :to=" { name : 'app-university-dashboard' } " class="block pl-3 pr-4 py-2 border-l-4 border-indigo-500 text-base font-medium text-indigo-700 bg-indigo-50 focus:outline-none focus:text-indigo-800 focus:bg-indigo-100 focus:border-indigo-700 transition duration-150 ease-in-out">Dashboard</nuxt-link>
